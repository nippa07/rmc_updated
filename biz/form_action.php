<?php
	...
	if (empty($uid) || empty($email) || empty($pwd) || empty($pwdRepeat)) {
	  ...
	  // We check for an invalid e-mail.
		...
		header("Location: bizsignup.php?error=invalidmail");
		exit();
	  }
	  // We check if the repeated pwd is NOT the same.
	  else if ($pwd !== $pwdRepeat) {
		header("Location: bizsignup.php?error=pwdcheck");
		exit(); 
	  }
                ...
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    ...
                    if ($rows > 0) {
                        ...
                        if (!mysqli_stmt_prepare($stmt, $sql)) {
                            ...
                            $rows= mysqli_fetch_assoc($result);
                            if ($rows > 0) {
                                $sql= "UPDATE users SET uidUsers = ?, pwdUsers = ?, bname = ?, fname = ?, lname = ?, phone = ?, emailUsers = ?, web = ?, city = ?, state = ?, zip = ?, claimed = ? WHERE id = ?";
                                $stmt = mysqli_stmt_init($conn);
                                mysqli_stmt_prepare($stmt, $sql);
                                $hashedPwd = password_hash($pwd, PASSWORD_DEFAULT);
                                mysqli_stmt_bind_param($stmt, "sssssssssssss", $uid, $hashedPwd, $bname, $fname, $lname, $phone, $email, $web, $city, $state, $zip, $claimed, $id);
								mysqli_stmt_execute($stmt);
								header('Refresh: 0; url= bizsignin.php');
                                    exit();
                            } else {
                                $sql = "INSERT INTO users (`uidUsers`,`pwdUsers`,`bname`,`fname`,`lname`,`phone`,`emailUsers`,`web`,`city`,`state`,`zip`,`claimed`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
                    ...
                    // If there is no error then we continue the script!
                                    ...
                                    // Next we need to bind the type of parameters we expect to pass into the statement, and bind the data from the user.
                                    mysqli_stmt_bind_param($stmt, "ssssssssssss", $uid, $hashedPwd, $bname, $fname, $lname, $phone, $email, $web, $city, $state, $zip, $claimed);
                                    // Then we execute the prepared statement and send it to the database!
